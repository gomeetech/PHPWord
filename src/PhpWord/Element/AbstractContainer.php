...
abstract class AbstractContainer extends AbstractElement
{
    ...
    public function __call($function, $args)
    {
        ...
        foreach ($elements as $element) {
            ...
            $replaces['replace' . strtolower($element)] = $element == 'Object' ? 'OLEObject' : $element;
        }
        ...
        if (isset($functions[$function])) {
        ...
        } elseif (isset($replaces[$function])) {
            ...
            if ($element == 'TextBreak') {
                ...
                for ($i = 1; $i <= $count; $i++) {
                    $this->replaceElement($element, $index, $fontStyle, $paragraphStyle);
                }
            } else {
                // All other elements
                array_unshift($args, $element); // Prepend element name to the beginning of args array

                return call_user_func_array(array($this, 'replaceElement'), $args);
            }
        }
    ...
    protected function addElement($elementName)
    {
        ...
        $element->setElementIndex($this->countElements());
     ...
     * @return \PhpOffice\PhpWord\Element\AbstractElement
     */
    protected function replaceElement($elementName, $index)
    {
        $elementClass = __NAMESPACE__ . '\\' . $elementName;
        $this->checkValidity($elementName);
        ...
        $withoutP = in_array($this->container, array('TextRun', 'Footnote', 'Endnote', 'ListItemRun', 'Field'));
        if ($withoutP && ($elementName == 'Text' || $elementName == 'PreserveText')) {
            $args[4] = null; // Remove paragraph style for texts in textrun
        }
        ...
        $elementArgs = $args;
        array_shift($elementArgs); // Shift the $elementName off the beginning of array
        array_shift($elementArgs); // Shift the index off the beginning of array
     ...
     * @return \PhpOffice\PhpWord\Element\AbstractElement
     */
    protected function insertElement($elementName, $index = -1)
    {
        $elementClass = __NAMESPACE__ . '\\' . $elementName;
        $this->checkValidity($elementName);
        ...
        $arrSplice = array_splice($this->elements, $index, count($this->elements) - $index);
        if ($arrSplice && count($arrSplice)) {
            foreach ($arrSplice as $elem) {
                $index++;
    ...
    public function removeElement($toRemove)
    {
        if (is_int($toRemove) && array_key_exists($toRemove, $this->elements)) {
            ...
            array_splice($this->elements, $toRemove, 1);
